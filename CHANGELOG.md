...
Auto-detect-from-website feature removed from both the new-project wizard and the in-project dashboard. The detector used to POST the homepage URL to `/api/detect-segment`, run it through an Anthropic call that inferred (industry, category, subcategory, primary product, region hint, seed keywords, suggested competitors), and offer the result for one-click acceptance. In practice the suggestions weren't accurate enough to act on without manual editing — users either skipped detection entirely or accepted it then immediately reworked the suggestions on the dashboard — and the call added 15–30 seconds of latency to project creation while burning Anthropic tokens on output that usually got discarded. Removed at user request.

### Changed
- **`app/projects/new/page.tsx`** — rewritten as a single-step form. Previous wizard had three phases (`input` → `detecting` → `review`) with a stepper indicator, detection spinner, segment card, competitor checkbox list, and seed-keyword chip list. Replaced with a single form collecting client URL, brand name, optional aliases, and region; submit creates the project and redirects to the dashboard. Effectively makes the old "Skip detection" path the only path. Removed the `DetectionResult` type, the `Phase` state, the `onDetect()` / `onSkipDetection()` / `onCreateProject()` triple, the `StepDot` / `ConfidenceChip` / `hostnameFrom` helpers, and the import of `SuggestedCompetitor` from SmartSegmentDetector. File shrank from 513 lines to 117.
- **`components/ProjectHeader.tsx`** — dropped the `<SmartSegmentDetector>` block (the "Market segment / Detect from website" panel from the user's screenshot), its `<div className="mt-5" style={{ paddingTop: 16, borderTop... }}>` wrapper, and the `onAutoSave` callback. The `seg` / `setSeg` state, the `seedsDirty` check, and the matching dirty-checks in the dirty-flag derivation were removed. The save() PATCH no longer sends `segment_l1/l2/l3`, `primary_product`, `custom_seed_keywords`, or `detection_confidence` — the underlying DB columns are untouched, so existing values on the row are preserved on update (the `lib/db.ts:updateProject` builds dynamic SET clauses only for keys present in the patch). Removed the `onCompetitorsSuggested` and `onSeedKeywordsApplied` props from the component signature since their only consumer (SmartSegmentDetector) is gone. Tooltip on the Save button reworded to reflect the new editable surface.
- **`components/Dashboard.tsx`** — stopped passing `onCompetitorsSuggested` and `onSeedKeywordsApplied` to `<ProjectHeader>`. Removed the now-dead `applySeedKeywords()` helper that the detector used to drive (its only call site was the prop chain). The `suggestedCompetitors` state and `persistSuggestions()` helper are KEPT — they're still populated from `project.suggested_competitors` on load and consumed by `CompetitorPanel` as `suggested={...}`, so any prior detector-generated suggestions that were persisted to the DB still render in the panel and can be added/dismissed.

### Not changed
- **`components/SmartSegmentDetector.tsx`** — left on disk as dead code. The component file isn't imported by anything in the new codebase (grep confirms zero callers after this release), but keeping it means re-enabling the detector later is a one-import line in `ProjectHeader.tsx` instead of a 230-line component rewrite.
- **`/api/detect-segment` route** and **`lib/llm.ts` segment-detection wiring** — left on disk. The route is still callable directly, just no longer exercised by any UI. Costs nothing while it sits idle.
- **`components/SegmentPicker.tsx`** — already a thin re-export of SmartSegmentDetector for legacy callers; left intact.
- **Project DB schema** — `segment_l1`, `segment_l2`, `segment_l3`, `primary_product`, `custom_seed_keywords`, `detection_confidence`, and `suggested_competitors` columns are unchanged. Historical projects that had values in these columns still load fine; the values just aren't shown or edited in the UI. The PPT prompt builder (`lib/export.ts`) still reads `project.segment_l1/l2/l3` for the `universe_label` slide title fallback, so a project that was created back when detection was enabled still gets its detected segment name in the slide title.
- **Other dashboard panels** unchanged (CompetitorPanel still works for manual competitor adds, KeywordPanel still works for manual keyword adds and CSV import, all other surfaces untouched).

### Notes
- Per user preference: the underlying detection output was already AI-generated (modeled rather than measured) and the user kept getting empty / inaccurate suggestions, so removing it aligns with "actual and factual data only" as the standard for what surfaces in the UI.
- The new-project wizard now has the same minimal-input shape as a typical SaaS onboarding step: name, URL, region, done. Adding tracked competitors and seed keywords is explicitly deferred to the dashboard panels.
...
### Changed
- **`app/api/projects/[id]/refresh/route.ts`** — rewritten as a chunked endpoint. POST now accepts two new optional query params: `snapshotId` (UUID of an in-flight snapshot to continue) and `chunk` (0-indexed chunk number). The first POST (no params) creates a new snapshot row and processes chunk 0; subsequent POSTs reuse the snapshot and process the next slice. The final chunk calls `finalizeSnapshot()` with cumulative AIO counts pulled via `snapshotProgress(snapshotId)` — that's the SAME query the polling widget uses, so the "Snapshot saved — N AIO(s) detected" banner the user sees at the end of a multi-chunk run matches the numbers the metrics endpoint will return on the next load. Defaults: `DEFAULT_CHUNK_SIZE = 200` tasks, `MAX_CHUNK_SIZE = 400` (a buggy caller can't request a 5000-task chunk and re-trigger the timeout). Concurrency stays at 4 — increasing it would mostly trade Vercel timeouts for SerpAPI rate-limit 429s, which is a worse failure mode.
- **`components/Dashboard.tsx` `onRefresh()`** — rewritten as a sequential chunk loop. Step 1: POST `/refresh` with no params to start chunk 0 and read `snapshot_id` + `total_chunks` off the response. Step 2: if `total_chunks > 1`, iterate `POST /refresh?snapshotId=X&chunk=N` for N=1..total_chunks-1 until `is_final`. Step 3: surface the cumulative AIO count + the chunk count in the success banner ("processed in N chunks to avoid Vercel's 300s function cap"). The polling effect that drives `/refresh/progress` is UNCHANGED — it was already reading serp_results row counts, which tick up monotonically across chunks because every chunk inserts into the same snapshot. Defensive JSON parsing (handles HTML error pages, Vercel timeouts, missing API key 500s, etc.) was lifted into a `postChunk()` helper so it applies to every chunk POST, not just the first.
- **`app/api/projects/[id]/refresh/progress/route.ts`** — `ZOMBIE_THRESHOLD_SEC` raised from 600 (10 min) to 3600 (60 min). A multi-chunk refresh keeps the snapshot in 'running' for the full duration of all chunks combined — at ~3 min per 200-task chunk, a 4000-task universe takes ~60 minutes. The new threshold accommodates that while still auto-failing genuinely dead invocations within an hour. Per-chunk stall detection (no new serp_results in 60s) is unchanged — it catches dead chunks on a much tighter timeline regardless of the overall cap.
- **`lib/db.ts`** — added `getSnapshot(id)` helper. Used by the refresh route to validate that the `snapshotId` echoed back by the client (a) exists, (b) belongs to the project being refreshed, and (c) is still in 'running' status. Without this guard a malicious or buggy client could append work to a completed snapshot from a different project.

### Not changed
- **Polling widget UI.** `RefreshProgress.tsx` reads the same `/refresh/progress` response shape. It now smoothly progresses from 0% to 100% across multiple chunk POSTs instead of in one big single-call run — visually indistinguishable from before, just doesn't 504 partway through.
- **Backward compat for small universes.** A POST with no params and a universe of ≤200 tasks completes in one call with `is_final: true` on the first response, and the response shape still carries `aios_triggered` and `failed` at the top level for any legacy caller that reads them. Existing integration tests / direct API users that didn't know about chunking keep working.
- **No data, API breaking changes, or schema migration.** The new query params on `/refresh` are optional. `snapshot_id`, `keywords_count`, `aios_triggered`, `status`, `error` columns on the `snapshots` table are unchanged.

### Notes
- Per user preference: every chunk's cumulative counts come from the SAME `snapshotProgress(snapshot_id)` query the polling widget uses (`COUNT(*) FROM serp_results WHERE snapshot_id = ?`). Nothing is estimated, modeled, or aggregated client-side. The numbers in the success banner are the literal DB counts after the final chunk completes.
- For a typical 600-keyword × single-region universe at the new default chunk size, expect 3 chunks × ~150-180s = ~8-9 minutes total wall-clock. For 1500 keywords single-region, expect 8 chunks × ~150s = ~20 minutes. Both well inside the 60-minute zombie threshold.
- If a chunk fails mid-stream (server returns 500 with `{ error }`), the snapshot is finalized as 'failed' and the next refresh starts a fresh snapshot — no partial state to clean up by hand.
- Hard-reload after deploy so the browser picks up the new client bundle. After the deploy, the v1.1.64 prompt (Copy PPT Prompt button) still emits the v1.1.63 3-slide + brand comparison output; only the refresh path changed.
...
### Changed
- **`lib/export.ts`** — `buildPptPrompt(latest, ctx, keywords?)` gained an optional third parameter for keyword-detail rows (`PptPromptKeywordRow[]`). The function now emits three slides:
  - **Slide 1 — AIO landscape.** Replaced the previous "7-card horizontal strip + 3-column SoV / non-brand chart" layout with the dashboard's actual `StoryPanel` strip: 2 hero KPI cards on row 1 (Available AIOs, AIO Penetration), 6 metric cards on row 2 (Your position, Brand mentions, Citation share, Avg citation position, Top brand · {leader}, Others). The slide-1 KPI numbers now use the SAME formulas the pulse cards use — `clientShare = aios_acquired / total_keywords` (not `citation_slots / total_citation_slots`), `mentionShare = mention_count / total_keywords`, `othersShare = bucket-sum slots / total_citation_slots`, `avg_citation_position` lifted straight from `latest.brands[client].avg_citation_position`, and the "Your position" subtitle uses the same `ordinal(rank) behind {leader}` phrasing — so a stakeholder reading the slide can verify against the live dashboard and have the numbers tie exactly.
  - **Slide 2 — AIO opportunity map.** Unchanged in structure (3 summary cards on top + cluster grid + THE READ orange callout). Title now respects pluralization of "cluster(s)".
  - **Slide 3 — Keyword-level opportunity & AIO drill-down (new).** Two-column layout. Left = top-17 keyword data table generated by interleaving the wins-bucket (sorted by `citations.length` desc) with the losses-bucket (same sort), so the slide leads with biggest wins / biggest misses side-by-side rather than an all-wins or all-losses ranking. Each row carries cluster · cites · top brand · client status (green `#N` chip when CHIP is cited, orange MENTION chip when mentioned-not-cited, red MISSING chip when absent). Right = two AIO drill-down panels: BEST WIN (highest CHIP rank tie-broken by citation count, panel border-top green, AIO query + answer text + top-5 citations with the ★ CHIP row marked) and WORST MISS (highest citation-count keyword where CHIP is absent, panel border-top red, same drilldown shape). The slide gracefully degrades to a "SKIP THIS SLIDE" stub when the keyword parameter is null or contains no AIO-triggered keywords, so older callers and brand-new snapshots don't break the prompt.
- New exported type **`PptPromptKeywordRow`** describing the minimal keyword-detail shape slide 3 needs (keyword, cluster, has_aio, aio_text, citations[], brand_hits[], winner). The full `/keywords/detail` response carries more fields (URLs, source_type, etc.) but the prompt only quotes a couple of rows back, so the surface is kept tight.
- New helpers in `lib/export.ts`: `trimAio(text, max=600)` to bound the AIO answer text quoted on slide 3 so the clipboard payload stays reasonable, and `ordinal(n)` lifted from `StoryPanel.tsx` so the "Your position" subtitle reads identically to the pulse card.
- **`components/Dashboard.tsx`** — `copyPptPromptHandler` now fetches `/api/projects/{id}/keywords/detail` (respecting the current region and `kindFilter` scope) before calling `buildPptPrompt`, and forwards the rows as the new third argument. The fetch uses the same three-layer cache bypass (`cache: "no-store"` + `_=<timestamp>` query param + the route's own `Cache-Control: no-store`) the `KeywordExplorer` already uses, so the prompt always reflects the just-landed snapshot. A failed fetch is non-fatal: it logs a console warning and lets the builder render slides 1 + 2 with a slide-3 fallback stub.
- **`components/HeaderActions.tsx`** — Copy PPT Prompt tooltip rewritten to name all three slides ("AIO Landscape, Cluster Opportunity Map, and Keyword-Level Opportunity + AIO Drill-Down") so users hovering the button know exactly what they're about to paste before they click.

### Not changed
- **No data, API, or metrics changes.** Slide 3 reads from the existing `/api/projects/[id]/keywords/detail` endpoint with the same params the `KeywordExplorer` panel already calls; no new route, no new SQL, no schema migration.
- **`lib/export.ts` PDF / CSV exporters** untouched. Only the PPT prompt builder block was modified.
- **Backward compatibility.** `buildPptPrompt(latest, ctx)` (2-arg signature) still works — the new keywords parameter defaults to `null`, and the builder emits a SKIP stub for slide 3 in that case. Anything that imported `buildPptPrompt` and was passing just two arguments compiles and runs unchanged.

### Notes
- Per user preference: every number on every slide is derived from the SAME real metrics + keyword-detail rows the dashboard renders. Nothing is modeled, simulated, or estimated. The selection rule for slide 3's top 17 ("highest citation count, interleaved wins + losses") was confirmed with the user before implementation.
- The receiving AI is still told to MATCH THE ACTIVE DECK for typography, color, and card styles — no theme is baked into the prompt. If a stakeholder wants a specific theme baked in, attach the .pptx and re-trigger; v1.1.62 keeps the "match the deck" instruction intact across all three slides.
- Hard-reload after deploy so the browser picks up the new client bundle.
